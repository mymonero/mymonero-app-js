...
class NavigationBarView extends View {
  ...
  setup () {
    ...
    { // background decoration view
      ...
      layer.classList.add('nav-bar-background')
    ...
    }
    {
      ...
      //
      layer.classList.add('nav-title-label')
      //
  ...
  SetTitleNeedsUpdate (stackView) {
    ...
    self.titleLayer.style.width = `calc(70% - 32px - ${extra_paddingLeft}px)`
  ...
  SetTitleFromTopStackView (stackView, old_topStackView, isAnimated, ifAnimated_isFromRightNotLeft, trueIfPoppingToRoot) {
    ...
    const to_styleWidth = `calc(70% - 32px - ${extra_paddingLeft}px)`
    //
    if (isAnimated === false) {
      self.titleLayer.style.color = titleTextColor
      self.titleLayer.innerHTML = emoji_web.NativeEmojiTextToImageBackedEmojiText_orUnlessDisabled_NativeEmojiText(
    ...
    const successor_final_left = `${parentWidth * 0.15 + 16}px`
    Animate(
      ...
      },
      {
        ...
        complete: function () {
          successor_titleLayer.style.left = 'calc(15% + 16px)'
